...
export default function ProductDetails({ product }: ProductDetailsProps) {
    // const [product, setProduct] = useState<Product | null>(initialData); // Use props directly preferably
    // const [loading, setLoading] = useState(false); // No loading state needed for initial data
    ...
    useEffect(() => {
        if (product) {
            ...
            if (product.fragrance) setSelectedFragrance(product.fragrance._id);
            if (product.format) setSelectedFormat(product.format._id);

            // If main product doesn't have them but variants do (edge case), try first variant
            if ((!product.fragrance || !product.format) && product.variants && product.variants.length > 0) {
                const firstV = product.variants[0];
                if (!product.fragrance && firstV.fragrance) setSelectedFragrance(firstV.fragrance._id);
                if (!product.format && firstV.format) setSelectedFormat(firstV.format._id);
            }
        }
    }, [product]);
                        ...
                        <div className="sticky top-24 flex flex-col md:flex-row gap-4">
                            ...
                            <div className="flex-1 relative">
                                <div className="relative w-full aspect-square md:aspect-[4/5] lg:aspect-square bg-transparent rounded-lg overflow-hidden">
                                    ...
                                    <Image
                                        src={activeImage || displayImages[0] || '/placeholder.png'}
                                        alt={product.name}
                                        fill
